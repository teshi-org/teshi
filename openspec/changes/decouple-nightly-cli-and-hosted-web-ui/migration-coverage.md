...
preview/event queues are saturated. Real Chromium acceptance against a
deployed `https://teshi.org/app/` and sidecar stream exercise remain explicit
follow-up evidence because they require a live Pages deployment and supported
Windows sidecars.
...
A live Chromium probe on 2026-09-09 reached `https://teshi.org/app/`, but the
deployed route currently serves the Hugo marketing document rather than the
GPUI WASM shell: the fragment remains visible and `/app/ui-manifest.json`
returns the same HTML fallback instead of a manifest. Therefore the real
hosted-page acceptance items 9.1-9.4 remain blocked on the Pages deployment;
this is an external deployment-state blocker, not a local daemon test result.
