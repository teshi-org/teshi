...
use teshi_engine::{
    ...
    BrowserMode, HostEventCallback, RuntimeConfig, TeshiEngine,
};
...
use crate::commands::{
    ...
    get_step_binding_statuses_cmd, highlight_locator_cmd, list_dir, open_project, open_project_dir,
    read_file_base64, read_text_file, reject_locator_cmd, render_feature_cmd, resize_terminal,
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            if let Some(path) = initial_project {
                let rt = app.state::<Arc<TeshiEngine>>().inner().clone();
...
fn init_logging() {
    let log_dir = teshi_engine::app_data_dir()
        .map(|p| p.join("logs"))
