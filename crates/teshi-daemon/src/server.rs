...
pub async fn run_server(
    ...
    let protected_routes = Router::new()
        ...
        .route("/api/v1/requirements/generate", post(api_requirements_generate))
...
async fn api_requirements_generate(
    ...
    let (api_key, base_url, model) =
        teshi_runtime::llm::llm_config_from_env().map_err(|e| ApiError::internal(e))?;
    ...
    // Call LLM
    let result = teshi_runtime::llm::call_llm_with_tool(
        &api_key,
        &base_url,
        &model,
        &system_prompt,
...
fn validate_mindmap_xml(xml: &str) -> Result<(), ApiError> {
    ...
    if !xml.contains("<map") || !xml.contains("</map>") {
        return Err("Mindmap XML must contain a <map> root element".to_string().into());
    }
    Ok(())
}
