...
//! Pure DTOs and normalisation live in `teshi-core::locator`.
...
use teshi_core::parse_feature;

pub use teshi_core::locator::{
    normalize_step_text, sanitize_feature_path, ActiveStep, HighlightInfo, LocatorCandidate,
    LocatorPrimary, PendingLocator, StepBinding, StepBindingStatus, StepBindingsFile,
...
impl LocatorWatcherState {
    ...
    pub fn watch_project(&self, project_root: &Path, rt: Arc<TeshiEngine>) -> Result<()> {
        self.clear()?;
...
fn emit_pending_locator(rt: &TeshiEngine, project_root: &Path) {
    let path = pending_locator_path(project_root);
    if !path.exists() {
        rt.events
...
fn emit_active_step_from_disk(rt: &TeshiEngine, project_root: &Path) {
...
pub async fn highlight_locator(rt: &TeshiEngine, selector: String) -> Result<(), String> {
...
pub fn get_active_step(rt: &TeshiEngine) -> Result<Option<ActiveStep>, String> {
    let project_root = rt
        .project
        .root
        .lock()
...
pub fn get_pending_locator(rt: &TeshiEngine) -> Result<Option<PendingLocator>, String> {
    let project_root = rt
        .project
        .root
        .lock()
...
pub async fn reject_locator(rt: &TeshiEngine) -> Result<(), String> {
    let project_root = rt
        .project
        .root
        .lock()
...
pub async fn abandon_pending_locator(rt: &TeshiEngine) -> Result<(), String> {
