...
use teshi_core::gherkin_lang::GherkinLanguage;
...
impl EditorBuffer {
    ...
    /// Extract text from `(start_row, start_col)` to `(end_row, end_col)` inclusive.
    ///
    /// The two positions are normalised so the earlier position comes first
    /// regardless of the argument order. When the positions are equal, an
    ...
    pub fn text_range(
        ...
        let mut lines: Vec<String> = Vec::with_capacity(hi_row - lo_row + 1);
        for row in lo_row..=hi_row {
            let line = self.line(row);
            let lo = if row == lo_row { lo_col } else { 0 };
            let hi = if row == hi_row {
                hi_col.min(line.chars().count())
            } else {
                line.chars().count()
            };
            let sliced: String = line.chars().skip(lo).take(hi.saturating_sub(lo)).collect();
            lines.push(sliced);
        }
        lines.join("\n")
    }
...
mod tests {
    ...
    #[test]
    fn test_text_range_same_position_returns_empty() {
        let buffer = EditorBuffer::from_string("hello\nworld".to_string());
        assert_eq!(buffer.text_range((0, 0), (0, 0)), "");
        assert_eq!(buffer.text_range((1, 0), (1, 0)), "");
    }

    #[test]
    fn test_text_range_single_line() {
        let buffer = EditorBuffer::from_string("hello world".to_string());
        assert_eq!(buffer.text_range((0, 0), (0, 5)), "hello");
        assert_eq!(buffer.text_range((0, 6), (0, 11)), "world");
    }

    #[test]
    fn test_text_range_multi_line() {
        let buffer = EditorBuffer::from_string("aaa\nbbb\nccc".to_string());
        assert_eq!(buffer.text_range((0, 1), (2, 2)), "aa\nbbb\ncc");
    }

    #[test]
    fn test_text_range_reversed_order() {
        let buffer = EditorBuffer::from_string("aaa\nbbb".to_string());
        assert_eq!(buffer.text_range((1, 1), (0, 1)), "aa\nb");
    }

    #[test]
    fn test_text_range_full_lines() {
        let buffer = EditorBuffer::from_string("line1\nline2\nline3".to_string());
        assert_eq!(buffer.text_range((0, 0), (2, 5)), "line1\nline2\nline3");
    }
}
