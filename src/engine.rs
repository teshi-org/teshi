...
pub fn load_engine_config(
    ...
    if config_path.exists() {
        ...
        if let Some(cfg) = file.engine {
            if let Some(cmd) = cfg.cmd.filter(|s| !s.trim().is_empty()) {
                return Ok(EngineConfig {
...
pub fn bindings_to_engine_steps(
    ...
    let bindings = teshi_runtime::list_step_bindings(project_root, feature_relative_path)?;
    ...
    if auto_propose && !recorded_steps.is_empty() {
        ...
        if let Some(first_step) = recorded_steps.first() {
            ...
            if let Some(loc) = locator {
                ...
                let pending = teshi_runtime::PendingLocator {
                    step_ref: teshi_runtime::ActiveStep {
                    ...
                    candidates: vec![teshi_runtime::LocatorCandidate {
                        rank: 1,
                ...
                teshi_runtime::propose_locator(&project_root, pending)?;
