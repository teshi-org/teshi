...
fn resolve_user_agent_dir(loader: &AgentLoader) -> Option<PathBuf> {
    #[cfg(test)]
    if let Some(ref dir) = loader.user_agent_dir_override {
...
impl Resolver {
    ...
    fn resolve_one(&mut self, id: &str) -> Option<AgentDefinition> {
        ...
        let raw = match self.raws.get(id) {
            Some(r) => r.clone(),
            None => return None,
        };
        let agent_dir = match self.dirs.get(id) {
            Some(d) => d.clone(),
            None => return None,
        };
