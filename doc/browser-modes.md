...
Terminal agents verify locators with `teshi browser execute --selector <css> --action <action>` and `--value-arg <text>` for `fill`, `type`, `assert_text`, `select`, and `press_key`; use `assert_not_exists` for a negative element-existence assertion (hidden matching elements still count as existing). These are the same flags as `teshi steps propose`. For RVP audit trail, use `teshi browser verify --step-line N ...` (highlight + execute + append to `.teshi/logs/locator-verify.jsonl`).
